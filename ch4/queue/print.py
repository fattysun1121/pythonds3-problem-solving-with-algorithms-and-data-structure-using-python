...
class Task:
    def __init__(self, time):
        self.timestamp = time
        self.pages = random.randrange(1, 11)  # Print load cut in half.
...
def new_print_task(num_students):
    tasks_per_sec_num = num_students * 2
    tasks_per_sec_den = 60 * 60
    seconds_per_task = tasks_per_sec_den // tasks_per_sec_num
    num = random.randrange(1, seconds_per_task + 1)
    return num == seconds_per_task


for i in range(10):
    simulation(3600, 20, 5)
